(() => {
  ...
  function render(){
    ...
    const displayGlyph = w.kanji || w.hiragana;
    el.kanjiDisplay.textContent = displayGlyph;
    el.kanjiDisplay.style.fontSize = displayGlyph.length > 4 ? '32px' : (displayGlyph.length > 2 ? '42px' : '56px');
    el.hiraganaDisplay.textContent = w.kanji ? w.hiragana : '';
    el.hiraganaDisplay.style.visibility = w.kanji ? 'visible' : 'hidden';
